...
public:
	...
	inline std::size_t pop(std::vector<Ptr> &op)
	{
		...
		if(ticket <= 0)
			return 0;
		index_type nticket = m_ticket.fetch_add(-ticket);
		if(nticket <= 0)
		{
...
private:
	...
	cursor_type		m_pptr;
	char			m_padding_1[g_cache_line-sizeof(index_type)];
	cursor_type		m_gptr;
	char			m_padding_2[g_cache_line-sizeof(cursor_type)];
	cursor_type		m_ticket;
	char			m_padding_3[g_cache_line-sizeof(cursor_type)];
	index_type		m_arity;
	index_type		m_mask;
	char			m_padding_4[g_cache_line-sizeof(index_type)*2];
